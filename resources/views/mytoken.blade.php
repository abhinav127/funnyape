     ...
     @include('menu')
      <div class="first-2">
       ...
       <div data-w-id="89f095dc-caf7-46ab-58a4-eb2582532690" style="transform: translate3d(0px, 0px, 0px) scale3d(1, 1, 1) rotateX(0deg) rotateY(0deg) rotateZ(0deg) skew(0deg); opacity: 1; transform-style: preserve-3d;" class="main-description-2">
          <div class="subtitle-bold inmob">My NFT Item</div>

        </div>
      </div>

      <div class="homecards-block">
        <div class="homecards-wrap nft-cards" id="item_list">
...
function getOwner()
{
  ...
  var contract = new web3.eth.Contract(abi,contract_addr);
       ...
       contract.methods.balanceOf(currentAccount).call().then((result)=>{
          for (let ti = 0; ti < result; ti++) {
             contract.methods.tokenOfOwnerByIndex(currentAccount,ti).call().then((tokenID)=>{
             console.log(tokenID);
             $("#item_list").append('<div class="home-nft">            <div class="nft__img__wrap"><img src="<?php echo url('ape'); ?>/'+tokenID+'.png" loading="lazy" sizes="(max-width: 479px) 61vw, 256px" srcset="<?php echo url('ape'); ?>/'+tokenID+'.png 500w, <?php echo url('ape'); ?>/'+tokenID+'.png 600w" alt="" class="nft__img-2"></div><div class="nft__info-2"> <div class="nft__header"> <p class="home-nft-name">Funny Ape #'+tokenID+'</p> </div> <div class="nft__price__wrap-2"> <div class="home-nft-eth"><button onclick="gotoopensea('+tokenID+')" class="link-5">GO&nbsp;ON&nbsp;OPENSEA</button> </div> </div> </div> </div>')
      })
            } 
     })
...
function gotoopensea(tokenID)
{
  window.location.href='https://testnets.opensea.io/assets/0x19857089994444cd4b0fd6885882ed861949ce36/'+tokenID;
}
